...
def average_true_range(df, n):
    ...
    TR_l = [0]
    while i < df.index[-1]:
        TR = max(df.loc[i + 1, "High"], df.loc[i, "Close"]) - min(
...
def average_directional_movement_index(df, n, n_ADX):
    ...
    ADX = pd.Series(
        ...
        .ewm(span=n_ADX, min_periods=n_ADX)
        .mean(),
...
def vortex_indicator(df, n):
    ...
    TR = [0]
    while i < df.index[-1]:
        Range = max(df.loc[i + 1, "High"], df.loc[i, "Close"]) - min(
            df.loc[i + 1, "Low"], df.loc[i, "Close"]
        )
        TR.append(Range)
        i = i + 1
    ...
    VM = [0]
    while i < df.index[-1]:
        Range = abs(df.loc[i + 1, "High"] - df.loc[i, "Low"]) - abs(
            df.loc[i + 1, "Low"] - df.loc[i, "High"]
        )
        VM.append(Range)
        i = i + 1
    VI = pd.Series(
...
def kst_oscillator(df, r1, r2, r3, r4, n1, n2, n3, n4):
    ...
    KST = pd.Series(
        ROC1.rolling(n1).sum()
        + ROC2.rolling(n2).sum() * 2
        + ROC3.rolling(n3).sum() * 3
        + ROC4.rolling(n4).sum() * 4,
        name="KST_"
        + str(r1)
        + "_"
        + str(r2)
...
def chaikin_oscillator(df):
    ...
    Chaikin = pd.Series(
        ad.ewm(span=3, min_periods=3).mean() - ad.ewm(span=10, min_periods=10).mean(),
        name="Chaikin",
    )
    df = df.join(Chaikin)
    return df
...
def commodity_channel_index(df, n):
    ...
    PP = (df["High"] + df["Low"] + df["Close"]) / 3
    CCI = pd.Series(
        (PP - PP.rolling(n, min_periods=n).mean()) / PP.rolling(n, min_periods=n).std(),
        name="CCI_" + str(n),
    )
    df = df.join(CCI)
    return df
...
def keltner_channel(df, n):
    ...
    """
    KelChM = pd.Series(
        ((df["High"] + df["Low"] + df["Close"]) / 3).rolling(n, min_periods=n).mean(),
        name="KelChM_" + str(n),
    )
    KelChU = pd.Series(
...
def ultimate_oscillator(df):
    ...
    while i < df.index[-1]:
        ...
        i = i + 1
    UltO = pd.Series(
        (4 * pd.Series(BP_l).rolling(7).sum() / pd.Series(TR_l).rolling(7).sum())
